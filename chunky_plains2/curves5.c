...
void QuadraticFormula( double a, double b, double c, double *l0,double *l1 )
{
	*l0 = (-b + sqrt( (b*b) - (4*a*c) ) ) / (2*a);
...
int main()
{
	...
	double d_a_x, d_a_y,d_b_x, d_b_y;
	double d_v_x,d_v_y;
		...
		if (win)
		{
			...
			for (z=0;z<2400;z++)
			{
				...
				for (p=0;p<matrix_max;p++)
				{
					...
					d_a_x = _matrix -> b -> x - ha.x;
					d_a_y = _matrix -> b -> y - ha.y;

					d_b_x =  hb.x - _matrix -> b -> x;
					d_b_y =  hb.y - _matrix -> b -> y;

					d_v_x = d_b_x - d_a_x;
					d_v_y = d_b_y - d_a_y;

					for (l=0.0f;l<1.0f;l+=0.1f)
					{
						x = ( (l*l) * d_v_x ) + (l *2*d_a_x) + ha.x;
						y = ( (l*l) * d_v_y ) + (l *2*d_a_y) + ha.y;

						set_pixel_fast(cp, x, y );
					}

//					QuadraticFormula( d_v_x, 2*d_a_x, ha.x - x, &l0, &l1 );
//					QuadraticFormula( d_v_y, 2*d_a_y, ha.y - y, &l0, &l1 );
				}
