...
//		bpr = p96GetBitMapAttr(  win -> RPort -> BitMap, P96BMA_BYTESPERROW);
		...
		if (err = alloc_cunky_plains(win -> RPort -> BitMap, 4,2,2,0,0,0,0,0, screenw,screenh,&cp))
		{
			printf("error code: %d\n",err);
		}
		else
		{
			...
			for (rot=0;rot<1000;rot++)
			{
				Delay(1);
				// show buffer
				memcpy( ptr, cp->data,cp -> bytes_per_row * screenh );
				...
				for (x=0;x<300;x++)
				{
					BltBitMapRastPort( scroll_rp.BitMap , x, 0, &scroll_rp, x, sin( (float) x / 100 *PI) * 50 + 120  , 1, 15,0x0C0 );
				}

				// clear buffer
//				bzero(cp -> data, cp -> bytes_per_row * 200); 

				memcpy( cp->data, p96GetBitMapAttr( bitmap , P96BMA_MEMORY) + (cp -> bytes_per_row * 20 ) ,cp -> bytes_per_row * screenh );
...
/*
				star(cp,g,100,100,50,100,0,3);
				star(cp,-g,100,100,100,50,1,2);
*/
			}
